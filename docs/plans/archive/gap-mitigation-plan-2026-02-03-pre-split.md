...
> **Last Updated**: 2026-02-02 | **Status**: PT-11 Phase 1 COMPLETE, Phase 2 COMPLETE
> **Plan Archive**: `docs/plans/archive/paired-animation-plan-2026-01-31-pre-restructure.md`
> **Latest Commit**: 7295922 - PT-11 Phase 2 Step 2: Extract timeline view to separate widget
> **Active Branch**: `feature/pt-11-preview-tool-refactor`

---

## EXECUTIVE SUMMARY: Priority Restructured

### ⭐ IMMEDIATE PRIORITY: Preview Tool Enhancements (Phase 5d)

The SPairedAnimationPreview editor tool is the **critical enabler** for all paired animation work. Enhancing it now will accelerate iteration on finishers, counters, and parries.

**Why Preview Tool First?**
1. **Iteration Speed**: Faster previewing = faster animation authoring
2. **Spatial Relationship**: ✅ FIXED - Tool now infers and constrains by spatial relationship
3. **Holistic Analysis Bug**: ✅ FIXED - Optimization now uses Time parameter correctly
4. **Visualization Gap**: No bone trajectory view - can't see where things will be over time

**Current State**:
- ✅ Montage section selection dropdowns (attacker/victim)
- ✅ Holistic timeline analysis with activity-weighted optimization
- ✅ Multi-contact point analysis
- ✅ Optimization results consistent across frames (PT-1 COMPLETE)
- ✅ Spatial relationship context for optimizer (PT-2 COMPLETE)
- ✅ Bone chain trajectory visualization (PT-3/PT-4 COMPLETE)
- ✅ Individual bone trajectory selection (PT-5 COMPLETE)
- ✅ Joint constraint analysis framework (PT-6/PT-7 COMPLETE)
- ✅ Weapon mesh attachment & visualization (PT-8/PT-9 COMPLETE)

### Runtime Finisher System: COMPLETE (~95%)

**Status**: All critical bugs fixed. Ready for manual testing.

✅ Finisher triggers below 20% health
✅ Victim dies and ragdolls directly (no double death animation)
✅ Player input unblocked after finisher completes
✅ Damage applied with intelligent calculation

### Priority Order (Restructured)

| Priority | Phase | Focus | Status |
|----------|-------|-------|--------|
| **1** | **5d** | **Preview Tool Enhancements** | ✅ ~95% COMPLETE |
| **2** | **5d-refactor** | **Preview Tool Architecture Refactor (PT-11)** | 🔄 IN PROGRESS |
| 3 | 5c | Math & Skeletal Utilities | ✅ COMPLETE (83 functions) |
| 4 | 5e | AnimInstance Integration | ⏳ NEXT |
| 5 | 5f | Awareness Subsystem | 🔮 Deferred |

### Gap Summary

**Total Gaps**: 143 (121 runtime + 22 preview tool) | **Done**: 51+ (11 PT complete) | **Pending**: 78 (13 PT pending) | **Deferred**: 14

---

## PHASE 5d: PREVIEW TOOL ENHANCEMENTS (IMMEDIATE PRIORITY)

> **Goal**: Make SPairedAnimationPreview the best-in-class editor tool for authoring paired animations.
> **Files**: `Source/KatanaCombatEditor/Public/PairedAnimationPreview.h`, `Private/PairedAnimationPreview.cpp`
> **Refactor Target**: Extract utilities → `PairedAnimationPreviewUtils.h/.cpp`, Config → `PairedAnimationPreviewConfig.h`

### Why This Phase First

1. **Iteration Enabler**: Every paired animation (finisher, counter, parry) requires tuning in this tool
2. **Current Pain Points**: Optimization inconsistency, no spatial context, no trajectory visualization
3. **Foundation for Runtime**: Editor-time analysis informs runtime procedural corrections
4. **User Request**: Explicitly asked to prioritize preview tool over runtime systems

### Phase 5d-1: Fix Optimization Consistency (PT-1) - P0

**Problem**: "Run Full Optimization" produces different results at different frames. This contradicts holistic analysis.

**Root Cause**: Optimization may be using current frame state instead of cached holistic analysis.

**Solution**:
1. Cache `FHolisticTimelineAnalysis` result on montage change
2. Ensure `OptimizeDistance()` and `OptimizeRotation()` use cached result exclusively
3. Disable timeline scrubbing during optimization (lock state)
4. Add visual indicator showing optimization scope ("Analyzing X frames with Y contact points")

**Files to Modify**:
- `PairedAnimationPreview.cpp`: `RunFullOptimization()`, `RebuildHolisticAnalysis()`

### Phase 5d-2: Spatial Relationship System (PT-2) - P0

**Problem**: Optimizer has no concept of intended spatial relationship. It doesn't know if animation is:
- **Facing**: Characters meant to face each other (front attacks)
- **Behind**: Attacker behind victim (backstabs)
- **Side**: Attacker approaching from left/right

**Research Finding (Agent a0b92de)**: Spatial relationship IS inferable from animation data:
1. `ContactNormal` direction at sync point indicates attack direction
2. Victim bone receiving contact indicates facing (spine = back, chest = front)
3. `VictimFacingMode` field already exists (-1 = face attacker, 1 = face away)

**Solution - Hybrid Approach**:
1. **Infer Suggested Relationship** from sync point contact location + victim bone
...
{
    Inferred,       // Auto-detect from animation
...
|--------------|-----------------------------------------------|-----------|
| Facing | 180° (facing attacker) | ±30° |
| Behind | 0° (back to attacker) | ±30° |
| LeftSide | 90° (left side to attacker) | ±30° |
| RightSide | -90° (right side to attacker) | ±30° |

**Files to Modify**:
- `PairedAnimationPreview.h`: Add `ESpatialRelationship`, `InferSpatialRelationship()`, dropdown state
- `PairedAnimationPreview.cpp`: Add inference logic, constrain optimization search space

### Phase 5d-3: Bone Chain Trajectory Visualization (PT-3/PT-4) - P1

**Problem**: Tool samples full animation but doesn't visually communicate this. Users can't see where bones will be over time.

**Solution - Bone Chain Ribbons**:
1. **Define Standard Chains**: LeftArm, RightArm, LeftLeg, RightLeg, Spine, Neck
2. **Sample Each Chain**: Extract positions at all sample times
3. **Draw Trajectory Ribbons**: Connect bone positions as lines/ribbons
4. **Color Gradient**: Blue (t=0) → Red (t=end) for temporal understanding
5. **Toggle Controls**: Per-chain enable, per-character enable

**Bone Chain Definitions** (from existing SkeletalAnalysisLibrary):
```
...
**New UI Elements**:
- Checkbox group: "Show Bone Chains" with sub-checkboxes for each chain
- Per-character toggle (Attacker chains / Victim chains)
- Opacity slider for trajectory lines

**Files to Modify**:
- `PairedAnimationPreview.h`: Add chain enable state, trajectory cache structs
- `PairedAnimationPreview.cpp`: Add `DrawBoneChainTrajectory()`, chain sampling

### Phase 5d-4: Individual Bone Trajectory Visualization (PT-5) - P1

**Problem**: Sometimes need finer-grained control than chains.

**Solution - Skeletal Hierarchy Dropdown**:
1. Add expandable tree view of skeleton for each character
2. Checkbox selection for individual bones
3. Same ribbon/gradient visualization as chains
4. Bone velocity display at cursor position

**Files to Modify**:
- `PairedAnimationPreview.cpp`: Add `DrawIndividualBoneTrajectory()`, tree view widget

### Phase 5d-5: Joint Constraint Visualization (PT-6/PT-7) - P2

**Problem**: No way to see if animations cause over-extension or over-compression of joints.

**Research Finding (Agent aa9c945)**: `FJointConstraint` struct exists but NO utility functions for angle analysis.

**Solution**:
1. **Define Joint Limits** (anatomical):
   - Elbow: 0° - 145° flexion
   - Knee: 0° - 140° flexion
   - Shoulder: -60° - 180° flexion
2. **Calculate Joint Angles**: Extract from parent-child bone transforms
3. **Visualize Violations**: Red highlight when angle exceeds limit
4. **Pole Vector Display**: Show IK pole positions for two-bone chains

**Files to Create**:
- Add `CalculateJointAngle()` to SkeletalAnalysisLibrary
- Add joint visualization to PairedAnimationPreview

### Phase 5d-Refactor: Architecture Cleanup (PRIORITIZE BEFORE MORE FEATURES)

**Problem**: PairedAnimationPreview.cpp is growing large (~2700 lines). Before adding more capabilities (bone trajectories, weapon meshes, read/write), we need to modularize.

**Why Refactor Now**:
1. **Prevent Monolithic File**: File already exceeds comfortable reading size
2. **Enable Multi-View Architecture**: Future views (trajectory editor, contact point editor) should share core logic
3. **Enable Read/Write Capability**: Clean separation makes data serialization easier
4. **Faster Iteration**: Changes to one system don't require understanding the entire file

**Future Vision: Preview → Editor**:
The tool should eventually support:
- **Authoring contact points** directly in the 3D viewport
- **Writing metadata** to montages (sync point markers, contact points)
- **Creating AnimNotifies** from predicted contacts (AnimNotifyState_PairedAnimationSync)
- **Editing PairedAnimationData** assets directly (VictimOffset, WarpConfig, etc.)
- **Frame-perfect workflow**: See the visualization, tweak the data, see updated results

**Proposed Architecture**:

```
Source/KatanaCombatEditor/
├── Public/
│   ├── PairedAnimationPreview.h              ← Main Slate widget (UI only)
│   ├── PairedAnimationPreviewConfig.h        ← Config struct + defaults (NEW)
│   ├── PairedAnimationPreviewModel.h         ← Data model/state (NEW)
│   ├── Analysis/
│   │   ├── PairedAnimationAnalysisEngine.h   ← Analysis functions (NEW)
│   │   ├── SpatialRelationshipAnalyzer.h     ← PT-2 spatial inference (NEW)
│   │   ├── TrajectoryAnalyzer.h              ← Bone trajectory sampling (NEW)
│   │   └── ContactPointAnalyzer.h            ← Contact detection (NEW)
│   ├── Views/
│   │   ├── IPairedAnimationView.h            ← View interface (NEW)
│   │   ├── SPairedAnimPreviewViewport.h      ← 3D viewport view (NEW)
│   │   ├── SPairedAnimTimelineView.h         ← Timeline/scrubber (NEW)
│   │   └── SPairedAnimAnalyticsView.h        ← Stats panel (NEW)
│   └── Serialization/
│       └── PairedAnimationDataWriter.h       ← Write to assets (FUTURE)
└── Private/
    ├── PairedAnimationPreview.cpp            ← Widget composition only
    ├── PairedAnimationPreviewConfig.cpp      ← Config defaults
    ├── PairedAnimationPreviewModel.cpp       ← Model state management
    ├── Analysis/
    │   ├── PairedAnimationAnalysisEngine.cpp ← Core analysis
    │   ├── SpatialRelationshipAnalyzer.cpp   ← Spatial inference
    │   ├── TrajectoryAnalyzer.cpp            ← Trajectory sampling
    │   └── ContactPointAnalyzer.cpp          ← Contact detection
    ├── Views/
    │   ├── SPairedAnimPreviewViewport.cpp    ← 3D viewport impl
    │   ├── SPairedAnimTimelineView.cpp       ← Timeline impl
    │   └── SPairedAnimAnalyticsView.cpp      ← Stats impl
    └── Serialization/
        └── PairedAnimationDataWriter.cpp     ← Asset writing (FUTURE)
```

**Refactor Order (Bottom-Up)**:
1. **Config** (`PairedAnimationPreviewConfig.h/.cpp`)
   - All magic numbers (thresholds, colors, sizes)
   - Bone chain definitions
   - Default socket names
   - Pattern: Same as `Debug/DebugConfig.h`

2. **Model** (`PairedAnimationPreviewModel.h/.cpp`)
   - Current state (montages, skeletons, time, playback state)
   - Character configs (AttackerConfig, VictimConfig)
   - Analysis cache state (bAnalysisCacheDirty, HolisticAnalysis)
   - Delegate declarations for state changes

3. **Analysis Engine** (`Analysis/*.h/.cpp`)
   - Extract `EvaluateConfigurationHolistic()`
   - Extract `ComputeMultiContactPoints()`
   - Extract `InferSpatialRelationship()`
   - Extract trajectory sampling functions
   - Pure functions operating on Model state

4. **Views** (`Views/*.h/.cpp`)
   - Extract viewport rendering
   - Extract timeline widget
   - Extract analytics panel
   - Each view observes Model via delegates

5. **Main Widget** (keep in `PairedAnimationPreview.h/.cpp`)
   - Widget composition (hosts views)
   - Tab spawner registration
   - Toolbar construction

**Target File Sizes Post-Refactor**:
- `PairedAnimationPreview.cpp`: ~500 lines (composition + toolbar)
- `PairedAnimationPreviewConfig.cpp`: ~100 lines (defaults)
- `PairedAnimationPreviewModel.cpp`: ~400 lines (state management)
- `PairedAnimationAnalysisEngine.cpp`: ~800 lines (core analysis)
- `SPairedAnimPreviewViewport.cpp`: ~600 lines (3D rendering)
- `SPairedAnimTimelineView.cpp`: ~300 lines (timeline)
- `SPairedAnimAnalyticsView.cpp`: ~300 lines (stats)

**Benefits**:
- Each file fits comfortably in context
- Analysis logic reusable by other tools
- Views can be composed differently (docked, floating)
- Read/write serialization cleanly separated
- Testing analysis functions in isolation

### Phase 5d-6: Weapon Mesh Attachment & Visualization (PT-8/PT-9) - P1

**Problem**: Preview tool only visualizes skeletal meshes. No weapon representation for:
- Weapon-to-body contact (finisher strikes)
- Weapon-to-weapon contact (parry visualization)
- IK requirements for weapon alignment

**Solution - Weapon Static Mesh Attachment**:
1. **Weapon Mesh Fields**: Add `AttackerWeaponMesh` and `VictimWeaponMesh` optional fields
2. **Socket Attachment**: Attach to configured weapon socket (e.g., `hand_r_weapon`)
3. **Weapon Socket Support**: Use `WeaponStart`/`WeaponEnd` sockets on weapon mesh if present
4. **Fallback to Bounds**: If no sockets, use mesh bounds or convex hull for contact detection
5. **Weapon Trajectory Visualization**: Show weapon path over time (critical for parry window analysis)

**Contact Point Enhancement**:
- **Weapon-to-Body**: Weapon mesh bounds → victim bone proximity
- **Weapon-to-Weapon**: Both weapon meshes → parry contact point prediction
- **IK Insight**: Calculate hand adjustment needed for weapon alignment with target

**UI Elements**:
- Checkbox: "Show Attached Weapons"
- Asset picker: Attacker/Victim weapon mesh selection
- Socket dropdown: Which skeleton socket to attach to
- Checkbox: "Use Weapon Sockets for Contact" (vs bounds)

**Files to Modify**:
- `PairedAnimationPreview.h`: Add weapon mesh references, socket names
- `PairedAnimationPreview.cpp`: Add weapon attachment, weapon-based contact detection

### Preview Tool Gap Summary

| Gap ID | Description | Priority | Status |
|--------|-------------|----------|--------|
| PT-1 | Optimization changes with frame (should be holistic) | P0 | ✅ COMPLETE |
| PT-2 | No spatial relationship context for optimization | P0 | ✅ COMPLETE |
| PT-3 | No visualization of full trajectory over time | P1 | ✅ COMPLETE |
| PT-4 | No bone chain visualization | P1 | ✅ COMPLETE |
| PT-5 | No individual bone trajectory selection | P1 | ✅ COMPLETE |
| PT-6 | No joint constraint analysis | P2 | ✅ COMPLETE |
| PT-7 | No pole vector visualization | P2 | ✅ COMPLETE |
| PT-8 | No weapon mesh attachment or visualization | P1 | ✅ COMPLETE |
| PT-9 | No weapon-to-weapon contact prediction | P1 | ✅ COMPLETE |
| **PT-10** | **Blocking operations on game thread (UI freezes on Optimize)** | **P1** | ✅ COMPLETE |
| **PT-11** | **"God Widget" anti-pattern (3000+ line monolithic widget)** | **P2** | ⏳ PENDING |
| PT-12 | Widget-managed component lifecycle (risky DestroyComponent) | P2 | ✅ COMPLETE |
| PT-13 | O(N) montage section lookup inefficiency | P3 | ✅ COMPLETE |
| PT-14 | Linear search optimization (should use binary/golden) | P2 | ✅ COMPLETE |
| PT-15 | Redundant cache invalidation on every slider tick | P2 | ✅ COMPLETE |
| PT-16 | Hardcoded UI configuration (colors, positions) | P3 | ✅ COMPLETE |
| PT-17 | String-based error filtering (fragile pattern) | P3 | ✅ COMPLETE |
| PT-18 | Boolean trap in FindOptimalRotation API | P3 | ✅ COMPLETE |
| **PT-19** | **Undo/redo support for optimization (history stack)** | **P0** | ✅ COMPLETE |
| PT-20 | Deprecation debt (AnimNotifyState_AttackPhase) | P3 | 🔮 DEFERRED (manual migration when ready) |
| **PT-21** | **Editor state change delegates (map/PIE safety)** | **P0** | ✅ COMPLETE |
| PT-22 | No visual feedback for long operations (no progress bar) | P2 | ✅ COMPLETE |
| PT-23 | Manual numeric formatting (use FText::AsNumber) | P3 | ✅ COMPLETE |
| PT-24 | Copy-paste code in socket handlers (DRY violation) | P2 | ✅ COMPLETE |

**Preview Tool Implementation Summary** (5,639 lines total):
- Weapon mesh attachment with configurable sockets (hand_r, weapon grip)
- Montage section selection with AnimNotify integration
- Holistic timeline analysis with activity-weighted optimization
- Spatial relationship inference from neutral configuration
- Multi-contact point analysis with weighted scoring
- Character orientation fix for weapon-forward alignment

### Preview Tool Code Review Gaps (PT-10 through PT-24)

Added 2026-02-02 from comprehensive code review. Categories: Architecture (A-C), Performance (D-F), Maintainability (G-I), Robustness (J-L), UX (M-O).

#### Critical (P0) - Safety Issues - ✅ IMPLEMENTED

**PT-19: Undo/Redo Support for Optimization** ✅
- **Issue**: `ApplyOptimizationResult()` modified widget state with no undo capability
- **Risk**: Users couldn't recover from bad optimization results
- **Solution**: Implemented custom history stack with `PushStateToHistory()`, `UndoOptimization()`, `RedoOptimization()`
- **Files**: `PairedAnimationPreview.h`, `PairedAnimationPreview.cpp`, `PairedAnimationEditorTypes.h`
- **Details**: Added `FPreviewOptimizationState` struct, 50-entry history stack, captures distance + rotations before each optimization

**PT-21: Editor State Change Delegates** ✅
- **Issue**: `UPairedAnimationAnalysisSubsystem` stored component pointers; no safety on world changes
- **Risk**: Crash on Map Change or PIE start/stop if context held stale pointers
- **Solution**: Bound `FEditorDelegates::MapChange`, `PreBeginPIE`, `EndPIE` to call `ClearContext()`
- **Files**: `PairedAnimationAnalysisSubsystem.h`, `PairedAnimationAnalysisSubsystem.cpp`
- **Details**: Added `OnMapChange()`, `OnPreBeginPIE()`, `OnEndPIE()` handlers; context auto-clears on world transitions

#### Major (P1) - Architectural - ✅ IMPLEMENTED

**PT-10: Blocking Operations on Game Thread** ✅
- **Issue**: `RunFullOptimization()` and `EvaluateConfigurationHolistic()` perform heavy synchronous calculations
- **Risk**: UI freezes for hundreds of ms or seconds during optimization
- **Solution**: Added `FScopedSlowTask` with progress dialog and cancellation support
- **Files**: `PairedAnimationPreview.cpp`, `PairedAnimationEditorTypes.h`
- **Details**: Progress feedback shows 5 phases (spatial analysis, distance, attacker rotation, victim rotation, sync time). User can cancel at any phase. Added `bWasCancelled` flag to `FOptimizationResult`. Also added `RunFullOptimizationWithProgress()` to subsystem for programmatic use.

#### Moderate (P2) - Architecture & Performance

**PT-11: "God Widget" Anti-Pattern** 🔄 IN PROGRESS
- **Issue**: `SPairedAnimationPreview` is 6,177 lines managing scene, viewport, UI, assets, events
- **Risk**: Difficult navigation, merge conflicts, tight coupling
- **Solution**: 2-phase refactor: (1) Extract Model + wire to existing Subsystem, (2) Extract View widgets
- **Branch**: `feature/pt-11-preview-tool-refactor`
- **Detailed Plan**: See "PT-11 Implementation Plan" section below

**PT-12: Widget-Managed Component Lifecycle** ✅
- **Issue**: `~SPairedAnimationPreview()` explicitly calls `DestroyComponent()` on mesh components
- **Risk**: Double-free or race conditions during editor shutdown (FAdvancedPreviewScene owns components)
- **Solution**: Removed `DestroyComponent()` calls; destructor now just nulls out pointers and lets `FAdvancedPreviewScene` handle cleanup
- **Files**: `PairedAnimationPreview.cpp`

**PT-14: Linear Search Optimization** ✅
- **Issue**: `FindOptimalDistance/Rotation` use linear iteration (O(N) with `Steps` parameter)
- **Risk**: High step count for precision kills performance
- **Solution**: Added `FindOptimalDistanceFast()` and `FindOptimalRotationFast()` using golden section search. O(log N) complexity vs O(N). Uses golden ratio (φ ≈ 1.618) to efficiently narrow search interval. Linear methods preserved for when curve data is needed for visualization.
- **Files**: `PairedAnimationAnalysisSubsystem.h`, `PairedAnimationAnalysisSubsystem.cpp`

**PT-15: Redundant Cache Invalidation** ✅
- **Issue**: `UpdateContextConfiguration` sets `bAnalysisCacheDirty = true` blindly every call
- **Risk**: Dragging sliders invalidates cache every tick, causing stuttering if UI redraws
- **Solution**: Added delta checks before invalidating - uses `FMath::IsNearlyEqual()` for floats and `FRotator::Equals()` for rotations
- **Files**: `PairedAnimationAnalysisSubsystem.cpp`

**PT-22: No Visual Feedback for Long Operations** ✅
- **Issue**: No progress bar or cancel option during long operations (optimization, cache rebuilds)
- **Risk**: UI freezes with no feedback; users can't cancel stuck operations
- **Solution**: Added `FScopedSlowTask` with progress dialogs and cancellation to ALL user-initiated long operations
- **Files**: `PairedAnimationPreview.h`, `PairedAnimationPreview.cpp`
- **Details**: Progress feedback added to:
  - `OnFindOptimalDistanceClicked()` - 50 distance samples with progress
  - `OnFindOptimalRotationClicked()` - 72 rotation samples (36 attacker + 36 victim)
  - `OnFindOptimalSyncClicked()` - Full analysis cache rebuild with per-frame progress
  - `OnGoToMaxSpeedClicked()` - Trajectory cache rebuild via `RebuildTrajectoryCacheWithProgress()`
  - "Rebuild Analysis Cache" button - Full rebuild of frame analysis + trajectories (~1500+ samples)
  - All operations show phase descriptions and can be cancelled at any step

**PT-24: Copy-Paste Code in Sockets** ✅
- **Issue**: `RefreshAttackerWeaponSocketOptions` and `RefreshVictimWeaponSocketOptions` have near-identical logic
- **Solution**: Created `RefreshWeaponSocketOptionsForCharacter(bool bIsAttacker)` helper; original methods now inline wrappers
- **Files**: `PairedAnimationPreview.h`, `PairedAnimationPreview.cpp`

#### Minor (P3) - Polish & Style

**PT-13: O(N) Montage Section Lookup** ✅
- **Issue**: `GetSectionTimeRange` iterates through all sections to find end time
- **Solution**: O(1) fast path checks `CompositeSections[SectionIndex + 1]` first; only falls back to O(N) linear search if sections are not chronologically ordered
- **Files**: `PairedAnimationPreview.cpp`, `AttackData.cpp` (both now fixed)

**PT-16: Hardcoded UI Configuration** ✅
- **Issue**: Colors, default positions, camera settings hardcoded in `.cpp`
- **Solution**: Created `PairedAnimPreviewConfig` namespace with categorized sub-namespaces (Camera, Character, SpatialConstraints, Colors, Sizes, Analysis) to centralize all magic numbers. Camera settings, debug colors, line widths, and analysis thresholds now reference config values. Prepares for future migration to `UDeveloperSettings` if Project Settings UI is desired.
- **Files**: `PairedAnimationPreview.cpp`

**PT-17: String-Based Error Filtering** ✅
- **Issue**: `IsDataValid` constructs string prefix to filter validation errors
- **Solution**: Added comprehensive documentation explaining the pattern, its limitations, and future improvement path. The current pattern works correctly but relies on error message formatting conventions. Full architectural refactor deferred as P3 - documented that future fix would require validation functions to use `FDataValidationContext` directly with sub-object associations.
- **Files**: `AttackData.cpp`

**PT-23: Manual Numeric Formatting** ✅
- **Issue**: Static helpers `GetNumberFormat0`, `GetNumberFormat1`, etc. duplicated logic
- **Solution**: Created unified `GetNumberFormat(int32 DecimalPlaces)` function with cached `FNumberFormattingOptions` array. Replaced all 17 call sites from `GetNumberFormatN()` to `GetNumberFormat(N)`.
- **Files**: `PairedAnimationPreview.cpp`

**PT-18: Boolean Trap in API** ✅
- **Issue**: `FindOptimalRotation(bool bOptimizeAttacker, ...)` is unclear at call site
- **Solution**: Created `EOptimizationTarget` enum with `Attacker`/`Victim` values; updated all signatures and call sites
- **Files**: `PairedAnimationEditorTypes.h`, `PairedAnimationAnalysisSubsystem.h`, `PairedAnimationAnalysisSubsystem.cpp`

**PT-20: Deprecation Technical Debt** 🔮 DEFERRED
- **Issue**: Support for deprecated `AnimNotifyState_AttackPhase` with no migration path
- **Current State**: `UE_LOG` warning in `AttackData.cpp` (lines 119-129) when deprecated notify detected
- **Proposed Solutions**: (a) `ensureMsgf` to break into debugger, or (b) "Fix It" button for auto-migration
- **Decision**: Defer - "Fix It" button adds complexity (~150+ lines of editor code) for limited benefit. When ready, manually identify affected montages via grep, convert by hand, then delete deprecated class. Focus on PT-11 refactor first.
- **Files**: `AttackData.cpp` (lines 119-129)

---

## PT-11 IMPLEMENTATION PLAN: "God Widget" Refactor

> **Status**: ✅ Phase 1 COMPLETE | 🔄 Phase 2 IN PROGRESS | **Branch**: `feature/pt-11-preview-tool-refactor`
> **Current Size**: ~5,500 lines (.cpp) + ~500 lines (.h) | **Target**: ~2,800 lines (.cpp)
> **Phase 1 Commits**: d2908b9, 3b111cf, 3707cee, 0fa97df, c5d9346

### Critical Discovery: Subsystem Already Exists

The `UPairedAnimationAnalysisSubsystem` already has all optimization/analysis APIs. The widget **duplicates** this logic rather than delegating:
...
**Root Cause**: Widget developed before subsystem was fully integrated.

### Recommended Strategy: 2-Phase Incremental Refactor

**Phase 1**: Extract Model + Wire to Subsystem (removes ~1,400 lines of duplicate code)
**Phase 2**: Extract View Widgets (viewport, timeline)

### Phase 1: Model Extraction

**Goal**: Encapsulate all preview state, wire widget to use existing subsystem for analysis.

#### Files to Create

**`PairedAnimationPreviewModel.h` (~200 lines)**
```cpp
USTRUCT()
struct FPairedAnimationPreviewModel
{
    // === Asset References ===
    TWeakObjectPtr<UAnimMontage> AttackerMontage, VictimMontage;
    TWeakObjectPtr<USkeletalMesh> AttackerSkeleton, VictimSkeleton;

    // === Configuration ===
    FCharacterPreviewConfig AttackerConfig, VictimConfig;
    FWeaponMeshConfig AttackerWeaponConfig, VictimWeaponConfig;
    ...
    // === Section Selection ===
    FName AttackerMontageSection, VictimMontageSection;
    float AttackerSectionStart, AttackerSectionEnd;
    float VictimSectionStart, VictimSectionEnd;
    ...
    // === Cached Results (moved from widget) ===
    TArray<FPairedFrameAnalysis> FrameAnalysisCache;
    TArray<FBoneTrajectory> AttackerTrajectories, VictimTrajectories;
...
#### What Moves from Widget to Model

From `.h` (lines 176-268): ~100 lines of member declarations
From `.cpp`: ~600 lines of cache management methods

#### Wiring to Existing Subsystem

```cpp
// BEFORE (duplicate in widget):
float Score = EvaluateConfigurationAtFrame(Distance, AttackerRot, VictimRot, Time);

// AFTER (delegating to existing subsystem):
UPairedAnimationAnalysisSubsystem* Subsystem = GEditor->GetEditorSubsystem<UPairedAnimationAnalysisSubsystem>();
FConfigurationEvaluationResult Result = Subsystem->EvaluateConfigurationAtFrame(Distance, AttackerRot, VictimRot, Time);
```

**Lines removed**: ~400 lines of duplicate optimization code (lines 2448-2850)

### Phase 2: View Extraction

**Goal**: Extract viewport and timeline into reusable Slate widgets.

#### Files to Create

**`SPairedAnimPreviewViewport.h/cpp` (~350 lines)**
- `FPairedPreviewViewportClient` (lines 154-188)
- `SPairedPreviewViewport` (lines 194-218)
- All `Draw*()` visualization methods (lines 3500-3780)

**`SPairedAnimTimelineView.h/cpp` (~200 lines)**
- Timeline slider and playback buttons
- `Tick()` playback logic
- Time display text logic

#### Interface Design

```cpp
class IPairedAnimationPreviewController
{
public:
    virtual FPairedAnimationPreviewModel& GetModel() = 0;
    virtual UDebugSkelMeshComponent* GetAttackerMesh() const = 0;
    virtual UDebugSkelMeshComponent* GetVictimMesh() const = 0;
    virtual void OnModelChanged() = 0;
    virtual void OnTimeChanged(float NewTime) = 0;
};
```

### File Size Estimates

| File | Before | After Phase 1 | After Phase 2 |
|------|--------|---------------|---------------|
| PairedAnimationPreview.h | 508 | ~300 | ~200 |
| PairedAnimationPreview.cpp | 6,177 | ~4,200 | ~2,800 |
| PairedAnimationPreviewModel.h | N/A | ~200 | ~200 |
| PairedAnimationPreviewModel.cpp | N/A | ~800 | ~800 |
| SPairedAnimPreviewViewport.h/cpp | N/A | N/A | ~430 |
| SPairedAnimTimelineView.h/cpp | N/A | N/A | ~260 |

### Implementation Order

```
Phase 1 Step 1: Create FPairedAnimationPreviewModel struct ✅ (d2908b9)
Phase 1 Step 2: Move state members from widget to model ✅ (3b111cf)
Phase 1 Step 3: Wire optimization to UPairedAnimationAnalysisSubsystem ✅ (3707cee)
Phase 1 Step 4: Move cache rebuild logic to model ✅ (0fa97df)
Phase 1 Cleanup: Remove duplicate internal methods ✅ (c5d9346)

Phase 2 Step 1: Extract FPairedPreviewViewportClient + SPairedPreviewViewport ✅ (e494da2)
Phase 2 Step 2: Extract SPairedAnimTimelineView ✅ (7295922)
```

### Verification Steps

**After Each Step**:
1. Build KatanaCombatEditor module
2. Open Paired Animation Preview tool
3. Verify: Asset selection, position/rotation changes, optimization buttons, undo/redo

**Final Verification**:
- Run all existing automated tests
- Compare optimization results before/after refactor (should be identical)

### Critical Files

| File | Purpose |
|------|---------|
| `PairedAnimationPreview.h/.cpp` | Widget to refactor |
| `PairedAnimationAnalysisSubsystem.h/.cpp` | Existing subsystem to delegate to |
| `PairedAnimationEditorTypes.h` | Existing types (model will use these) |
...
### Files Already Created (Foundation) - ALL COMPLETE

| File | Purpose | Status |
|------|---------|--------|
| `Math/CombatMathEnums.h` | Foundation enums (10 enums: EDistanceFormula, ESpatialQueryShape, EBoneChainType, etc.) | ✅ Complete |
| `Math/CombatMathTypes.h` | Foundation structs (10 structs: FSkeletalHierarchy, FBoneChain, FReachQueryResult, etc.) | ✅ Complete |

### Utility Libraries (Phase 5c) - ALL COMPLETE (3,128 lines, 83 functions)

| File | Lines | Functions | Status |
|------|-------|-----------|--------|
| `SkeletalAnalysisLibrary.h/.cpp` | 814 | 18 | ✅ Complete |
| `GeometryMathLibrary.h/.cpp` | 499 | 20 | ✅ Complete |
| `SpatialQueryLibrary.h/.cpp` | 706 | 15 | ✅ Complete |
| `PhysicsIntegrationLibrary.h/.cpp` | 610 | 15 | ✅ Complete |
| `PairedAnimationUtilityLibrary.h/.cpp` | 499 | 15 | ✅ Complete |

**Key Utility Functions Available**:
- Skeletal: `BuildSkeletalHierarchy()`, `GetBoneChain()`, `CalculateReachEnvelope()`, `CalculateCenterOfMass()`
- Geometry: Distance calculations (5 formulas), bounding volumes, convex hulls
- Spatial: Sphere/box/cone queries, FOV checks, spatial partitioning
- Physics: Verlet integration, trajectory prediction, collision prediction
- Paired: Contact point calculation, reach validation, obstacle detection
...
### GAP COVERAGE SUMMARY (Updated 2026-02-02 - Post Phase 5c/5d Completion)
...
| **Preview Tool (PT-x)** | **9** | **9** | **0** | **0** | **0** |
| **Phase 5c Math Libraries** | **5** | **5** | **0** | **0** | **0** |
| **TOTAL** | **135** | **49** | **0** | **72** | **14** |
...
**⭐ Phase 5c COMPLETE (2026-02-02)** - Math & Utility Libraries:
- ✅ SkeletalAnalysisLibrary: 18 functions (814 lines) - bone chains, reach envelopes, center of mass
- ✅ GeometryMathLibrary: 20 functions (499 lines) - distance calculations, bounding volumes
- ✅ SpatialQueryLibrary: 15 functions (706 lines) - sphere/box/cone queries, FOV checks
- ✅ PhysicsIntegrationLibrary: 15 functions (610 lines) - Verlet integration, trajectory prediction
- ✅ PairedAnimationUtilityLibrary: 15 functions (499 lines) - contact points, obstacle validation
- **Total**: 83 functions, 3,128 lines of production-ready utility code

**⭐ Phase 5d ~95% COMPLETE (2026-02-02)** - Preview Tool Enhancements:
- ✅ PT-1: Holistic optimization consistency (uses Time parameter correctly)
- ✅ PT-2: Spatial relationship inference from neutral configuration
- ✅ PT-3/PT-4: Bone chain trajectory visualization with time gradient
- ✅ PT-5: Individual bone trajectory selection via hierarchy tree
- ✅ PT-6/PT-7: Joint constraint analysis framework with anatomical limits
- ✅ PT-8/PT-9: Weapon mesh attachment with configurable grip sockets
- **Tool Size**: 5,639 lines (PairedAnimationPreview.cpp) + 1,796 lines (EditorTypes.h)
- **Latest Fix**: Commit 3beddea - Weapon grip socket + montage section looping fix
...
1. **20.5**: Sync point distance drift (characters drifting 90+ units during finisher) - P1
2. **19.6**: No fallback when attacker warp blocked by obstacle - P2
3. **AI Token System**: UCombatTokenSubsystem for attack coordination - P2
4. **AnimInstance Integration** (Phase 5e): Procedural IK targets - P2

**WORKING AS INTENDED**:
- **19.4**: Finisher uses SoftAimRange (finisher-specific detection had issues)
...
## Phase 5d Reference (Moved to Top)

> **Note**: Phase 5d: Preview Tool Enhancements has been moved to the **top of this plan** as the immediate priority.
> See "PHASE 5d: PREVIEW TOOL ENHANCEMENTS (IMMEDIATE PRIORITY)" section at the beginning of this document.
