...
`LightAttack_1` through `LightAttack_6` are asset-backed counter proof paths. They have:
...
| --- | --- | --- | --- |
| Already proven | `LightAttack_1` through `LightAttack_6` | Counter data, counter window, finisher data all present; batch 1 and batch 2 readiness and idempotence are clean | Do not re-migrate unless idempotence fails |
| Commandlet-safe light rollout | `LightAttack_7` through `LightAttack_11` | Post-batch full light rollout plan reports 5 would change, 0 failed, and 0 warning rows | Apply in the remaining `AM_Light_Combo_3` batch after review |
