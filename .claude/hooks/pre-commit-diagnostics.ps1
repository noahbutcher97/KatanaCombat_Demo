...
Write-Output "[DIAGNOSTICS] Running pre-commit diagnostics check..."
Write-Output ""

# Load diagnostics configuration
$configPath = ".claude/diagnostics-config.json"
if (-not (Test-Path $configPath)) {
    Write-Output "[WARNING] Diagnostics config not found at $configPath"
...
if (-not $stagedFiles) {
    Write-Output "`[INFO] No staged files to check"
    Write-Output "</system-reminder>"
    exit 0
}
...
if (-not $cppFiles) {
    Write-Output "`[INFO] No C++ files in staged changes"
    Write-Output "</system-reminder>"
    exit 0
}

Write-Output "`[CHECK] Checking $($cppFiles.Count) staged C++ files..."
Write-Output ""
...
Write-Output "`[TIP] To run full diagnostics check with filtering:"
...
Write-Output "`[CHECKLIST] Pre-commit Checklist:"
Write-Output ""

if ($hasComponent) {
    Write-Output "`[COMPONENT] CombatComponent files modified:"
...
if ($hasAnimNotify) {
    Write-Output "`[ANIM] AnimNotify files modified:"
...
if ($hasData) {
    Write-Output "`[DATA] Data asset files modified:"
...
if ($hasTests) {
    Write-Output "`[TEST] Test files modified:"
...
Write-Output "`[OK] General checklist:"
...
# Recommendation
Write-Output "`[RECOMMENDED] Recommended Actions:"
...
if ($Force) {
    Write-Output "[WARNING] Force mode enabled - skipping blocking"
    Write-Output "</system-reminder>"
    exit 0
}
...
Write-Output "`[TIP] Tip: Use '/hooks skip diagnostics' to disable this check"
Write-Output "</system-reminder>"
